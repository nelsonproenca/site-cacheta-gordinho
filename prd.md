...
- Confronto entre 2 contas de **streamers diferentes**: **não conta pontos em nenhum ranking** (nem de uma conta, nem da outra) — isolado do ranking regular, mesmo espírito de isolamento já aplicado ao Caxetão (4.5), só que mais estrito.
...
- Pareamento de 2 jogadores **dentro da mesma conta** (ex: dois jogadores da mesma live, ou do mesmo Caxetão) continua existindo como mecanismo separado, acessado a partir da própria tela do evento/conta — não passou por essa reformulação.
